                ...
                <div class="portlet-title">
                    ...
                    <div class="actions">
                        <div class="btn-group btn-group-devided" >
                            ...
                            <a href="{{url('admin/agent/'.$emloye->id.'/subtract')}}" class="btn btn-transparent grey-salsa btn-outline btn-circle btn-sm">
                        ...
                        <div class="btn-group">
                            <a class="btn red btn-outline btn-circle" href="{{url('admin/employees/salarys')}}" >
                                ...
                                <span class="hidden-xs"> {{trans('orbscope.all')}} {{trans('orbscope.salarys')}}</span>
                            </a>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="table-container">
                        <table class="table table-striped table-bordered table-hover table-checkable" id="datatable_ajax">
                            ...
                            <tr role="row" class="heading">
                                ...
                                <th width="10%"> ID OFFICE </th>
                                <th width="10%"> {{trans('orbscope.salary')}} </th>
                                <th width="25%"> {{trans('orbscope.note')}} </th>
                                <th width="5%"> {{trans('orbscope.status')}} </th>
                                <th width="10%">  {{trans('orbscope.update')}} </th>
                            </tr>
                            <form action="{{url('admin/add_salary/'.$emloye->id)}}" method="post">
                                {{ csrf_field() }}
                                ...
                                <td>
                                    <input type="text" class="form-control form-filter input-sm" required name="officer_id"> </td>
                                <td>
                                ...
                                <form  name="update_salary" action="{{url('admin/update_salary')}}" method="post">
                                @foreach($salary as $k=>$v)

                                        {{ csrf_field() }}
                                        <input type="hidden" name="salary_id[]" value="{{$v->id}}">
                                       <tr>
                                    <td>{{$k+1}}</td>
                                    <td>
                                        <div class="input-group date date-picker margin-bottom-5" data-date-format="yyyy-mm-dd">
                                            <input value="{{$v->date}}" type="text"  class="form-control  form-filter input-sm" required readonly name="date[]" >
                                            <span class="input-group-btn">
                                                                    <button class="btn btn-sm default" type="button">
                                                                        <i class="fa fa-calendar"></i>
                                                                    </button>
                                    ...
                                    <td>
                                        <input type="text" value="{{$v->id_office}}" class="form-control form-filter input-sm" required name="officer_id[]"> </td></td>
                                    <td>
                                        <input value="{{$v->salary}}" type="number" step="any" class="form-control form-filter input-sm" required name="salary[]">
                                    </td>
                                    <td>
                                        <input value="{{$v->note}}"  type="text" class="form-control form-filter input-sm" name="note[]" placeholder="" /></td>
                                    <td>
                                        <label class="mt-checkbox mt-checkbox-single mt-checkbox-outline">
