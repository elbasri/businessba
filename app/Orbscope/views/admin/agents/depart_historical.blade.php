                    ...
                    <div class="table-container">
                        <table class="table table-striped table-bordered table-hover table-checkable" id="datatable_ajax">
                            ...
                            <tr role="row" class="heading">
                                <th width="10%"> # </th>
                                <th width="30%"> {{trans('orbscope.department')}} </th>
                                <th width="30%"> {{trans('orbscope.date')}} </th>
                                <th width="30%">  {{trans('orbscope.update')}} </th>
                            </tr>
                            <form action="{{url('admin/add_depart_historic/'.$agent->id)}}" method="post">
                                {{ csrf_field() }}
                                <tr role="row" class="filter">
                                    ...
                                    <td>
                                    <select name="depart_id" required class="form-control select2">
                                        <option value=""></option>
                            ...
                            <form   action="{{url('admin/depart_historical')}}" method="post">

                                {{ csrf_field() }}
                                @foreach(@$agent->departHistoric as $k=>$h)
                                    <tr role="row" >
                                        ...
                                        <td>
                                            <select name="depart_id[]" required class="form-control select2">
                                                <option value=""></option>
                                                @foreach($branchs as $b)
                                                    <option value="{{$b->id}}" {{$h->depart_id ==$b->id?'selected':''}}>{{VarByLang($b->name,GetLanguage())}}</option>
                                                @endforeach
                                            </select>
                                        </td>
                                        <td>
