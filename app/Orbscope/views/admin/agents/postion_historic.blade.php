                    ...
                    <div class="table-container">
                        <table class="table table-striped table-bordered table-hover table-checkable" id="datatable_ajax">
                            ...
                            <tr role="row" class="heading">
                                <th width="10%"> # </th>
                                <th width="30%"> {{trans('orbscope.position')}} </th>
                                <th width="30%"> {{trans('orbscope.date')}} </th>
                                <th width="30%">  {{trans('orbscope.update')}} </th>
                            </tr>
                            <form action="{{url('admin/addPostion_historical/'.$agent->id)}}" method="post">
                                {{ csrf_field() }}
                                <tr role="row" class="filter">
                                    ...
                                    <td>
                                        <select name="position_id" required class="form-control select2">
                                            <option value=""></option>
                                            @foreach($postions as $b)
                            ...
                            <form  name="update_salary" action="{{url('admin/postion_historical')}}" method="post">

                                {{ csrf_field() }}
                                @foreach(@$agent->postionHistoric as $k=>$h)
                                    <tr role="row" >
                                        ...
                                        <td>
                                            <select name="position_id[]" required class="form-control select2">
                                                <option value=""></option>
                                                @foreach($postions as $b)
                                                    <option value="{{$b->id}}" {{$h->position_id ==$b->id?'selected':''}}>{{VarByLang($b->name,GetLanguage())}}</option>
                                                @endforeach
                                            </select>
                                        </td>
                                        <td>
